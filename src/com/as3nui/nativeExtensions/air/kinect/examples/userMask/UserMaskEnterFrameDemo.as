...
public class UserMaskEnterFrameDemo extends DemoBase {
	...
	private function calulateHeight(user:User):void{
		//var height:Number = user.head.position.depth.y + user.neck.position.depth.y+user.
	trace(	user.head.position.depth.length);
    ...
    protected function enterFrameHandler(event:Event):void {
        ...
        for each(var user:User in kinect.users) {
            ...
            if (user.userMaskData != null) {
				if(user.leftHand.position.depth.y < user.leftHip.position.depth.y){
					calulateHeight(user);
				}
